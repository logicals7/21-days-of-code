...
public class d8_sumOfTwoArrays {
	...
	public static int[] sumOfTwoArrays(int arr1[], int arr2[]) {	
		//sumArr should be 1 index larger than the maxSize(arr1 & arr2) to hold the carry.
		int sumArr[] = new int[arr1.length > arr2.length ? arr1.length+1 : arr2.length+1];
		System.out.println("size of the sumArr:" + sumArr.length);
		...
		//when one of the both arrays has elements remaining
		while( i >= 0 || j >=0 || k >= 0) {
			int sum = carry;
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < t; i++) {
			int arr1[] = takeInput();
			int arr2[] = takeInput();
			int sumArr[] = sumOfTwoArrays(arr1, arr2);
			printArray(sumArr);
		}
	}
