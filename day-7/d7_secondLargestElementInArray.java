...
public class d7_secondLargestElementInArray {
	...
	public static int secondLargestElementInArray(int arr[]) {
		...
		if(arr.length <= 1) {
			...
			for(int i = 0; i < arr.length; i++) {
				if(largest < arr[i]) {
					secondLargest = largest;  //secondLargest should be assign first before we reassign largest a new value
					largest = arr[i];
				}
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < t; i++) {
			int arr[] = takeInput();
			int secondLargest = secondLargestElementInArray(arr);
