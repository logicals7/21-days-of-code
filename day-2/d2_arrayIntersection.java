...
public class d2_arrayIntersection {
		...
		for(int i = 0; i < arr1.length; i++) {
			for(int j = 0; j < arr2.length; j++) {
				if(arr1[i] == arr2[j]) {
					...
					arr2[j] = Integer.MIN_VALUE; // replacing the already intersected value with minimum value in arr2, so that we don't print it twice. 
					break;
				}
			}
		}
